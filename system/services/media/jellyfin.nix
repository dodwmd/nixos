...

{
  options.homelab.media.jellyfin = {
    ...
    cachePath = mkOption {
      type = types.str;
      default = "/tank/data/transcodes";
      description = "Path to Jellyfin transcode cache";
    };
  ...
  config = mkIf config.homelab.media.jellyfin.enable {
    virtualisation.oci-containers.containers.jellyfin = {
      ...
      volumes = [
        ...
        "${config.homelab.media.jellyfin.cachePath}:/cache"
      ];
    ...
    systemd.services.podman-jellyfin.preStart = lib.mkAfter ''
      mkdir -p ${config.homelab.media.jellyfin.cachePath}
    '';
