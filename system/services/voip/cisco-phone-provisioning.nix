{
  ...
  in ''
    ...
    <authPassword>@AUTH_PASSWORD_${mac}@</authPassword>
      ...
      type = types.attrsOf (types.submodule {
        options = {
          ...
          authPasswordFile = mkOption {
            type = types.path;
            description = "Path to file containing the SIP authentication password (use an agenix secret)";
          };
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.cisco-phone-configs = {
      ...
      script = ''
        ...
        # Substitute runtime secrets into phone configs
        ${concatStringsSep "\n" (mapAttrsToList (_name: phone: ''
          AUTH_PASS=$(cat ${phone.authPasswordFile})
          ${pkgs.gnused}/bin/sed -i "s|@AUTH_PASSWORD_${phone.mac}@|$AUTH_PASS|g" "${cfg.tftpRoot}/SEP${phone.mac}.cnf.xml"
        '') cfg.phones)}
