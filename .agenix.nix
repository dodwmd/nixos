let
  ...
  exodus = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIKtH+LngDl7D89rGqi0G97XU61H8fRkJGsUKLbyQoLyb root@exodus";
...
in
{
  # Desktop secrets - exodus host + user key
  "secrets/wifi-something.age".publicKeys = users ++ [ exodus ];

  # VoIP secrets - k3s-worker-01 (hosts the TFTP server) + user key
  "secrets/sip-password.age".publicKeys = users ++ [ k3s-worker-01 ];
